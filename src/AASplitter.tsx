...
import html2canvas from "html2canvas"; // 需要 npm install html2canvas
...
export default function AASplitter() {
  ...
  const containerRef = useRef<HTMLDivElement>(null); // 用于导出图片
  ...
  async function exportImage() {
    ...
    link.download = "AA_Settlement.png";
    link.click();
  }

  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-50 via-white to-purple-100 p-4">
      <div ref={containerRef} className="max-w-3xl mx-auto bg-white/70 backdrop-blur-md rounded-3xl shadow-2xl p-5 sm:p-7">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-4">
          <input ref={nameRef} value={nameInput} onChange={e=>setNameInput(e.target.value)} onKeyDown={handleKeyDown}
            ...
            placeholder="姓名" />
          <input ref={amountRef} type="number" inputMode="decimal" value={amountInput} onChange={e=>setAmountInput(e.target.value)} onKeyDown={handleKeyDown} onFocus={handleAmountFocus}
        ...
        <div className="mt-5 flex flex-wrap gap-3 justify-center">
          ...
          <button onClick={exportImage} className="px-5 py-2 bg-gradient-to-r from-emerald-400 to-emerald-600 text-white rounded-lg hover:scale-105 transition-transform duration-200">
            导出图片
          </button>
        </div>
      </div>
